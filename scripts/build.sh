...
if [ ! -f "$INSTALL_DIR/lib/libzstd.a" ]; then
  ...
  cmake -S "$ROOTDIR/zstd-1.5.6/build/cmake" -B "$BUILD_DIR/zstd" \
    ...
    -DCMAKE_EXE_LINKER_FLAGS="$CROSS_LDFLAGS" -DCMAKE_SHARED_LINKER_FLAGS="$CROSS_LDFLAGS" -DCMAKE_SHARED_MODULE_LINKER_FLAGS="$CROSS_LDFLAGS" \
...
args=(
  ...
  -DCMAKE_SHARED_MODULE_LINKER_FLAGS="$CROSS_LDFLAGS"
